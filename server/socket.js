...
// ── stats ตัวละคร อ่านจาก config_character.js โดยตรง (ไม่ต้อง sync มือ) ──
const { CHARACTERS } = require('../client/js/config_character.js');
const CHARACTER_STATS = {};
CHARACTERS.forEach((c) => {
  CHARACTER_STATS[c.id] = c.stats || {};
});
function getCharRegenPct(charId) {
  const stats = CHARACTER_STATS[charId] || CHARACTER_STATS.default || {};
  return stats.regenPct || 0;
}
function getCharSpeedPct(charId) {
  const stats = CHARACTER_STATS[charId] || CHARACTER_STATS.default || {};
  return stats.speedPct || 0;
}
const REGEN_TICK_MS = 200; // ทุก 200ms
...
function initSocket(io) {
  io.on('connection', (socket) => {
    ...
    socket.on('update', (data) => {
      ...
      // [FIX #2] reducePct — รับจาก client แต่ clamp ไว้ที่ 0-100
      if (typeof data.reducePct === 'number') {
        p.reducePct = Math.min(100, Math.max(0, data.reducePct));
      }
      // armor แยกส่วน: bodyReducePct และ headReducePct
      if (typeof data.bodyReducePct === 'number') {
        p.bodyReducePct = Math.min(100, Math.max(0, data.bodyReducePct));
      }
      if (typeof data.headReducePct === 'number') {
        p.headReducePct = Math.min(100, Math.max(0, data.headReducePct));
      }
  ...
  // ── server-side regen tick: source of truth สำหรับ HP regen ──────────
  // เดิม regen ทำที่ client เท่านั้น → server ไม่รู้ hp ที่แท้จริง
  // ทำให้ผู้เล่นอื่นเห็น hp ผิด และอาจตายทั้งๆที่ hp จริงยังไม่หมด
  setInterval(() => {
    WORLD_IDS.forEach(wid => {
      const worldPlayers = players[wid];
      Object.keys(worldPlayers).forEach(pid => {
        ...
        const newHp = Math.min(p.maxHp, p.hp + (regenPerSec / 100) * REGEN_TICK_MS);
        if (newHp === p.hp) return;
        p.hp = newHp;

        io.to(wid).emit('player_update', {
          id:    pid,
          hp:    p.hp,
          alive: p.alive,
        });
